...
/** @defgroup bde_base64_encode_tests Tests */
...
/**
 * @addtogroup bde_base64_encode_tests
 * @{
 */
...
static void test_base64_encode_invalid_args(void ** state)
{
    ...
    error = bde_base64_encode(NULL, tds.decoded_len, tds.encoded_len, tds.encoded, &tds.encoded_len);
    assert_int_equal(bde_error_args, error);

    // Check that error is returned when the decoded buffer length given is not valid.
    error = bde_base64_encode(tds.decoded, 0U, tds.encoded_len, tds.encoded, &tds.encoded_len);
    assert_int_equal(bde_error_args, error);

    // Check that error is returned when the encoded buffer maximum length given is not valid.
    error = bde_base64_encode(tds.decoded, tds.decoded_len, 0U, tds.encoded, &tds.encoded_len);
    assert_int_equal(bde_error_args, error);

    // Check that error is returned when the encoded buffer given is not valid.
    error = bde_base64_encode(tds.decoded, tds.decoded_len, tds.encoded_len, NULL, &tds.encoded_len);
    assert_int_equal(bde_error_args, error);
}
...
static void test_base64_encode_invalid_buffer_length(void ** state)
{
    ...
    error = bde_base64_encode(sp->decoded, sp->decoded_len, sp->encoded_base64_len, tds.encoded, &tds.encoded_len);
...
static void test_base64_encode_ignored_buffer_length(void ** state)
{
    ...
    error = bde_base64_encode(sp->decoded, sp->decoded_len, tds.encoded_len, tds.encoded, NULL);
    assert_int_equal(bde_error_ok, error);
    assert_string_equal(sp->encoded_base64, tds.encoded);
}
...
static void test_base64_encode_two_separate_buffers(void ** state)
{
    ...
    for (size_t i = 0U; i < sample_count; i++)
    {
        ...
        // Encode.
        error =
            bde_base64_encode(sp->decoded, sp->decoded_len, sp->encoded_base64_len + 1U, tds.encoded, &tds.encoded_len);
        assert_int_equal(bde_error_ok, error);
        assert_int_equal(sp->encoded_base64_len, tds.encoded_len);
        assert_string_equal(sp->encoded_base64, tds.encoded);
    }
}
...
static void test_base64_encode_single_inplace_buffers(void ** state)
{
    ...
    for (size_t i = 0U; i < sample_count; i++)
    {
        ...
        // Encode.
        error = bde_base64_encode(tds.decoded,
                                  tds.decoded_len,
                                  sp->encoded_base64_len + 1U,
                                  (bde_enc_t *)tds.decoded,
                                  &tds.decoded_len);
        assert_int_equal(bde_error_ok, error);
        assert_int_equal(sp->encoded_base64_len, tds.decoded_len);
        assert_string_equal(sp->encoded_base64, (const bde_enc_t *)tds.decoded);
    }
}
...
static void test_base64_encode_length_invalid_args(void ** state)
{
    ...
    error = bde_base64_encode_length(0U, &tds.encoded_len);
    assert_int_equal(bde_error_args, error);

    // Check that an error is returned when the encoded length is not valid.
    error = bde_base64_encode_length(sp->decoded_len, NULL);
    assert_int_equal(bde_error_args, error);
}
...
static void test_base64_encode_length_samples(void ** state)
{
    ...
    for (size_t i = 0U; i < sample_count; i++)
    {
        ...
        const sample_t * const sp = sample_get(i);
        // Encode.
        error = bde_base64_encode_length(sp->decoded_len, &tds.encoded_len);
        assert_int_equal(bde_error_ok, error);
        assert_int_equal(sp->encoded_base64_len, tds.encoded_len);
    }
}
...
int main(void)
{
    ...
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_base64_encode_invalid_args, setup, teardown),
        cmocka_unit_test_setup_teardown(test_base64_encode_invalid_buffer_length, setup, teardown),
        cmocka_unit_test_setup_teardown(test_base64_encode_ignored_buffer_length, setup, teardown),
        cmocka_unit_test_setup_teardown(test_base64_encode_two_separate_buffers, setup, teardown),
        cmocka_unit_test_setup_teardown(test_base64_encode_single_inplace_buffers, setup, teardown),
        cmocka_unit_test_setup_teardown(test_base64_encode_length_invalid_args, setup, teardown),
        cmocka_unit_test_setup_teardown(test_base64_encode_length_samples, setup, teardown),
    };
